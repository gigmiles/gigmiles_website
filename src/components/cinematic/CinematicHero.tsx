...
import {CINEMATIC_ASSETS, CINEMATIC_CUES, CINEMATIC_SCENES, type SceneSpec} from './cinematic-cues'
...
function renderLine(text: string) {
  ...
  return parts.map((part, i) => (i % 2 === 1 ? <em key={i} className="cine-mark">{part}</em> : part))
}
...
export function CinematicHero({layout = 'portrait'}: {layout?: 'portrait' | 'landscape'}) {
  ...
  useEffect(() => {
    ...
    return installCinematic(root, video, {cues: CINEMATIC_CUES, src: {desktop: CINEMATIC_ASSETS.desktop, mobile: CINEMATIC_ASSETS.mobile}})
  }, [])
