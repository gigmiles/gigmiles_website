...
describe('approved bounded scroll story',()=>{
  ...
  it('pins each hero column to one viewport and scales the stage instead of leaving a band',()=>{
    const css=readFileSync('src/components/editorial/scroll-story.css','utf8')
    expect(css).toContain('@media(min-width:981px) and (min-height:660px) and (prefers-reduced-motion:no-preference)')
    expect(css).toContain('--story-runway')
    expect(css).toContain('min-height: calc(100svh + var(--story-runway))')
    expect(css).toContain('height: calc(100svh - 48px)')
    expect(css).toContain('justify-content: center')
    expect(css).toMatch(/\.stage \{\s*height: min\(clamp\(480px, 62svh, 760px\)/)
    expect(css).not.toMatch(/100vh/)
    const editorial=readFileSync('src/components/editorial/editorial.css','utf8')
    expect(editorial).toContain('overflow-x:clip')
    expect(editorial).not.toMatch(/overflow-x:\s*hidden/)
    expect(editorial).not.toMatch(/display:flex!important/)
    expect(editorial).toContain('grid-template-columns:minmax(0,240px) minmax(0,720px)')
    const responsive=readFileSync('src/components/editorial/responsive.css','utf8')
    expect(responsive).toContain('@media (min-width: 721px) and (max-width: 980px)')
    expect(responsive).toContain('.reading-layout { display: block; }')
    const layout=readFileSync('src/app/layout.tsx','utf8')
    expect(layout).toContain('viewportFit: "cover"')
    expect(layout).not.toMatch(/font-space-grotesk|font-dm-sans/)
  })
})
