...
export function ArticlePage({post,html,toc,related}:{post:BlogPost;html:string;toc:{id:string;text:string}[];related:BlogPost[]}){
 ...
 return <WebsiteShell paper>
  ...
  <div className="wrap reading-layout"><aside className="contents"><p>IN THIS GUIDE</p><nav aria-label="Article contents">{toc.map(h=><a key={h.id} href={'#'+h.id}>{h.text}</a>)}</nav><a className="text-link" href="/calculator">Try your own figures ↗</a></aside>
