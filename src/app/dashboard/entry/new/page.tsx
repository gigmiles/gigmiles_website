...
export default function NewEntryPage() {
    ...
    const onSubmit = async (data: z.infer<typeof schema>) => {
        ...
        try {
            ...
            // Success feedback
            const hasEarnings = earningsData.some((e) => e.amount > 0)
            const hasExpenses = expensesData.some((e) => e.amount > 0)
                            ...
                            <Card key={field.id} className="shadow-sm border-border/50 relative group">
                                <CardContent className="pt-6 flex gap-4 items-end">
                                    <div className="w-1/3 flex flex-col gap-2">
                                        ...
                                        {watch(`expenses.${index}.category`) === 'Other' && (
                                            <Input
                                                placeholder="Custom category name..."
                                                {...register(`expenses.${index}.customCategory`)}
                                            />
                                        )}
                                    </div>
                                    <div className="w-1/3">
